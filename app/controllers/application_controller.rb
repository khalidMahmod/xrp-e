class ApplicationController < ActionController::Base
  ...
  #before_action 'mfa_check',:except =>    [:index, :create, :verify]
  before_action :two_factor_check
  before_action :check_signature

  private

  def check_mfa
    if !(user_mfa_session = MemberMfaSession.find) && (user_mfa_session ? user_mfa_session.record == current_user : !user_mfa_session)
  ...
  def check_signature
    redirect = 0
    if current_user.present?
      if current_user.is_disabled == Member::DISABLED_ONCE
        if ((Time.now - current_user.disable_time)/60).floor >= 30
            current_user.update_attribute(:is_disabled, Member::ENABLED)
        else
            remaining_time = 30 - ((Time.now - current_user.disable_time)/60)
            msg = "You are blocked for 30 minutes. Please try after #{remaining_time.floor} min"
            redirect = 1
        end
      elsif current_user.is_disabled == Member::DISABLED_TWICE
        if ((Time.now - current_user.disable_time)/60).floor >= 60
            current_user.update_attribute(:is_disabled, Member::ENABLED)
        else
            remaining_time = 60 - ((Time.now - current_user.disable_time)/60)
            msg = "You are blocked for 60 minutes. Please try after #{remaining_time.floor} min"
            redirect = 1
        end
      elsif current_user.is_disabled == Member::DISABLED_PERMANENTLY
        msg = "You are blocked parmanently!"
        redirect = 1
      elsif current_user.signature != session[:signature]
        msg = "Token Mismatch!"
        redirect = 1
      end
      if redirect == 1
        lh =LoginHistory.where(member_id: current_user.id).last
        lh.update_attributes(:logouttime => DateTime.now )
        clear_all_sessions current_user.id
        reset_session
        redirect_to(root_path, alert: msg) and return
      end
      true
    end
  end
  ...
  def current_user
   ...
   # @current_user = Member.find_or_create_by(nickname: 'arfancse', email: 'arfancse@example.com')
  ...
  def set_gon
    ...
    gon.pusher = {
      ...
      cluster:   ENV.fetch('PUSHER_CLUSTER', 'ap2'),
  ...
  #
  #
  #   elsif (session[:member_id] > 0 && session[:check_varify_session] != "false" && session[:check_varify_session] != "true")
  #     redirect_to member_mfa_session_check_verify_path
  #   end
  # end
end
