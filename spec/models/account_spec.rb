describe Account do
  ...
  describe '#versions' do
    ...
    context 'when account add funds with fees' do
      subject { account.plus_funds('10'.to_d, fees: '1'.to_d, reason: Account::WITHDRAW).last_version }
    ...
    context 'when account sub funds with fees' do
      subject { account.sub_funds('10'.to_d, fees: '1'.to_d, reason: Account::WITHDRAW).last_version }
      it { expect(subject.reason.withdraw?).to be true }
      it { expect(subject.locked).to be_d '0' }
      it { expect(subject.balance).to be_d '-10' }
      it { expect(subject.amount).to be_d '90' }
    ...
    context 'when account unlock and sub funds with fees' do
      let(:account) { create(:account, balance: '10'.to_d, locked: '10'.to_d) }
      subject { account.unlock_and_sub_funds('10'.to_d, fees: '1'.to_d, locked: '10'.to_d, reason: Account::WITHDRAW).last_version }
      it { expect(subject.reason.withdraw?).to be true }
      it { expect(subject.locked).to be_d '-10' }
      it { expect(subject.balance).to be_d '0' }
      it { expect(subject.amount).to be_d '10.0' }
  ...
  describe 'after callback' do
    ...
    it 'should retry the whole transaction on stale object error' do
      # `unlock_and_sub_funds('5.0'.to_d, locked: '8.0'.to_d, fees: ZERO)`
